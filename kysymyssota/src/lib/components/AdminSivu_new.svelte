    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 gap-4">
        {#each filteredKysymykset as kysymys (kysymys.id)}
          <div 
            class="card p-4 backdrop-blur-md bg-white/10 dark:bg-black/10 border border-white/20 dark:border-white/10"
            class:border-red-400={kysymys.virhe}
            style={kysymys.virhe ? "box-shadow: 0 0 20px rgba(239, 68, 68, 0.5);" : ""}
          >
            <div class="flex items-center space-x-2 mb-3">
              <span class="text-lg">{vaikeustasoIcon(kysymys.vaikeustaso)}</span>
              {#if kysymys.virhe}
                <span class="text-red-500 text-lg" title="Kysymyksessä on ilmoitettu virhe">🚨</span>
              {/if}
              {#if kysymys.kategoria}
                <span class="badge variant-filled-secondary">{kysymys.kategoria}</span>
              {/if}
              <span class="badge variant-filled-primary">{kysymys.pistemaara_perus} pistettä</span>
              {#if kysymys.lahde === 'admin'}
                <span class="badge variant-filled-warning">Admin</span>
              {/if}
            </div>
            
            <h4 class="text-lg font-semibold mb-3">{kysymys.kysymys}</h4>
            
            <div class="space-y-2">
              <div class="text-sm">
                <span class="text-green-500 font-semibold">✓ Oikea vastaus:</span>
                <span class="ml-2">{kysymys.oikea_vastaus}</span>
              </div>
              
              <div class="text-sm">
                <span class="text-red-400 font-semibold">✗ Väärät vastaukset:</span>
                <div class="ml-4 mt-1">
                  {#each JSON.parse(kysymys.vaarat_vastaukset || '[]') as vaara, index}
                    <div class="text-red-300">• {vaara}</div>
                  {/each}
                </div>
              </div>
            </div>
